...
export class UserService {
  ...
  private url: string = 'http://localhost:4200/store/users';
  private urlLogin: string = 'http://localhost:4200/store/users/login';
  ...
  public insertFavoriteByUserNameAndItemId(userName: string, itemId: number) {
    let urlFavorite: string = "http://localhost:4200/store/users/" + userName + "/favorites/" + itemId;
    return this.httpCliente.put(urlFavorite, null); }
  public removeFavoriteByUserNameAndItemId(userName: string, itemId: number) {
    let urlFavorite: string = "http://localhost:4200/store/users/" + userName + "/favorites/remove/" + itemId;
    return this.httpCliente.delete(urlFavorite);
  }
  public getAllFavoritesByUserName(username: string): Observable<number[]> {
    let urlFavorite: string = "http://localhost:4200/store/users/" + username + "/favorites";
